 ...
 *
 * @author Rihad Jahan Opu <itrihad@gmail.com>
 * @version 0.1
 *
...
const PWA = ((PWA_CONFIG, notification) => {
      ...
      if (Navigator.isSupportedServiceWorker()) {
        window.addEventListener("load", () => {
          navigator.serviceWorker
            .register(PWA_CONFIG.sw.filepath)
            .then((registration) => {
            ...
            .catch((error) => {
              console.error(
                "PWA: Service Worker registration failed with " + error
              );
            });
        });
      }
  ...
  class Notification {
    ...
    static isDefault() {
      if (this.getPermission() == "default") {
        console.log(
          "PWA: Permission for Notifications are with default status."
        );
        return true;
      }
      return false;
            ...
            registration.pushManager
              .subscribe({
                ...
                applicationServerKey: Helper.urlBase64ToUint8Array(
                  PWA_CONFIG.push.server.public_key
                ),
              })
              .then((subscription) => {
  ...
  class Server {
    ...
    static sendSubscription(subscription, method) {
      ...
      return this.send(
        PWA_CONFIG.push.server.endpoint,
        method,
        JSON.stringify({
